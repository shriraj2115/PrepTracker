...
- **Analytics** — score trends, section-strength radar, weakness ranking, study-hours chart, readiness gauges, a mistake-type breakdown (Silly Error vs Concept Gap vs Time Pressure, etc.) with an actionable tip for whichever one is costing you the most marks, and an optional Error Log — a free-text note field on the score-entry screen for the rare mistake worth remembering (not every wrong answer), reviewable later and mark-as-reviewed when you've internalized it.
...
- **AI Assistant (optional, off by default)** — add your own [OpenRouter](https://openrouter.ai/keys) API key in Settings to unlock two things: a "🤖 Explain" button on each Error Log note that gives a specific root-cause + fix for that mistake, and a "✨ Get AI Insight" button on Analytics that writes one personalized observation from your actual accuracy/weak-topic/mistake data (upgrades the existing rule-based Insights list, doesn't replace it). Nothing calls out to the internet unless you click one of these buttons — every click spends from your own OpenRouter balance, so it's opt-in per request, not automatic. The key lives only in this browser's local storage.
